    ...
    <div className="max-w-[1440px] mx-auto px-5 md:px-16 py-6 md:py-8 space-y-8 text-[#dfe0ff]">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-white/10">
        <div className="flex gap-4">
          ...
          <button onClick={() => setActiveTab('lab')} className={`relative flex items-center gap-2 pb-2 font-bold text-sm transition-colors ${activeTab === 'lab' ? 'text-[#d846ef]' : 'text-[#8080a0]'}`}>
        ...
        {activeTab === 'reading' ? (
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-12 text-[#c6c5d4]">
            ...
            <motion.div variants={itemVariants} className="pb-4 space-y-2 border-b border-white/5">
              <div className="text-xs font-bold text-[#00daf3] tracking-wide uppercase">{turingData.chapter} / {turingData.part}</div>
              <h1 className="flex items-center gap-2 text-3xl font-extrabold text-white">
                <Brain className="text-[#00daf3]" size={28} /> {turingData.word_bn} <span className="text-lg font-normal text-[#8080a0]">({turingData.word_en})</span>
              </h1>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="grid items-start grid-cols-1 gap-8 md:grid-cols-12">
              <div className="space-y-4 md:col-span-7">
                <div className="flex items-center gap-2 text-[#d846ef] font-bold text-xs uppercase tracking-wider"><Sparkles size={16} />{turingData.real_world_flash.title}</div>
                {turingData.real_world_flash.paragraphs.map((p, idx) => <p key={idx} className="text-lg text-justify indent-6">{p}</p>)}
              </div>
              <div className="flex justify-center pt-2 md:col-span-5">
                <div className="w-full max-w-[320px] rounded-2xl bg-[#070512] border border-purple-500/20 p-4 shadow-[0_0_20px_rgba(168,85,247,0.15)]">
                  <div className="flex items-center justify-between pb-2 mb-3 border-b border-white/10">
                    <div className="flex gap-1.5 items-center text-xs font-mono font-bold text-purple-300"><Terminal size={14} /> IMITATION GAME TERMINAL</div>
                  ...
                  <div className="bg-black/40 border border-white/5 p-3 min-h-[160px] flex flex-col gap-2 rounded-lg relative">
                    {simDialogues.slice(0, imitationStage + 1).map((msg, i) => (
                      <div key={i} className={`flex flex-col ${msg.sender === 'judge' ? 'items-end' : msg.sender === 'system' ? 'items-center border-t border-purple-500/20 pt-1 mt-1' : 'items-start'}`}>
                        {msg.sender !== 'system' && <span className={`text-[10px] font-mono ${msg.sender === 'judge' ? 'text-purple-400' : msg.sender === 'roomX' ? 'text-emerald-400' : 'text-sky-400'}`}>{msg.sender.toUpperCase()}:</span>}
                        <span className={`text-xs px-2.5 py-1.5 rounded-lg border ${msg.sender === 'judge' ? 'bg-purple-950/40 border-purple-800/30 text-purple-200 rounded-tr-none' : msg.sender === 'system' ? 'border-none text-[10px] font-black text-purple-400 animate-pulse' : 'bg-emerald-950/40 border-emerald-800/30 text-emerald-200 rounded-tl-none'}`}>{msg.text}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <motion.div variants={itemVariants} className="p-6 rounded-xl border border-[#d846ef]/20 bg-gradient-to-r from-[#4f46e5]/3 to-[#d846ef]/3 shadow-md space-y-4">
              <h3 className="flex gap-2 text-lg font-bold text-white"><Sparkles className="text-[#d846ef]" size={18}/>{turingData.readers_reflection.title}</h3>
              <p className="font-serif italic">{turingData.readers_reflection.question}</p>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <button onClick={() => setPollSelected('A')} className={`p-4 rounded-xl border text-left flex gap-2.5 ${pollSelected === 'A' ? 'bg-green-500/10 border-green-500/40 text-white' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                  ...
                  <div><span className="block font-bold text-white">অপশন ১ (সঠিক)</span>হ্যাঁ, টুরিং টেস্ট পাস করেছে।</div>
                </button>
                <button onClick={() => setPollSelected('B')} className={`p-4 rounded-xl border text-left flex gap-2.5 ${pollSelected === 'B' ? 'bg-red-500/10 border-red-500/40 text-white' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                  ...
                  <div><span className="block font-bold text-white">অপশন ২</span>এক্সপার্ট সিস্টেম টেস্ট পাস করেছে।</div>
                </button>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="p-6 rounded-xl border border-[#d846ef]/20 bg-[#1c0c35]">
              <p className="text-[#c6c5d4]">উপরে থাকা <strong>"🔬 ল্যাব সিমুলেটর"</strong> ট্যাবে ক্লিক করে নিজে টুরিং টেস্টের বিচারক হোন!</p>
