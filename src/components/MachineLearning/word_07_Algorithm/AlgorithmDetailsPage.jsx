          ...
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-10 md:space-y-12 font-sans text-base sm:text-lg md:text-xl lg:text-[21px] leading-relaxed text-slate-300">
            ...
            <motion.div variants={itemVariants} className="pb-3 space-y-1 font-sans border-b md:pb-4 md:space-y-2 border-white/5">
              <div className="flex flex-wrap items-center gap-1.5 md:gap-2 text-[10px] md:text-xs font-bold text-slate-500 tracking-wide uppercase">
                <span className="hidden sm:inline">{algorithmData.part}</span>
              </div>
              <h1 className="flex flex-wrap items-center gap-2 text-2xl font-extrabold text-slate-100 sm:text-3xl">
                <GitCommit className="w-6 h-6 text-slate-400 md:w-7 md:h-7 shrink-0" /> 
                {algorithmData.word_bn} <span className="font-sans text-sm font-normal sm:text-lg text-slate-500">({algorithmData.word_en})</span>
              </h1>
            </motion.div>

            {/* --- Real World Flash & Cube Simulator --- */}
            <motion.div variants={itemVariants} className="grid items-start grid-cols-1 gap-6 md:gap-8 lg:grid-cols-12">
              <div className="space-y-3 lg:col-span-7 md:space-y-4">
                <div className="flex items-center gap-2 font-sans text-slate-400">
                  ...
                  <span className="text-sm font-bold leading-snug tracking-wider text-white uppercase sm:text-base md:text-lg">{algorithmData.real_world_flash.title}</span>
                </div>
                {algorithmData.real_world_flash.paragraphs.map((p, idx) => <p key={idx} className="text-justify indent-6">{p}</p>)}
              </div>
              <div className="flex justify-center pt-2 lg:col-span-5">
                <div className="w-full max-w-[280px] rounded-2xl bg-[#0b111b] border border-white/10 p-5 shadow-xl flex flex-col items-center mx-auto">
                  <div className="flex flex-col items-center justify-center p-2 mb-4 border border-dashed w-28 h-28 border-slate-500/30 rounded-xl">
                    <motion.div
                    ...
                    >
                      {[...Array(9)].map((_, i) => (
                        <div key={i} className={`w-4 h-4 rounded-sm transition-colors ${cubeSolved ? "bg-slate-400" : i % 3 === 0 ? "bg-slate-600" : i % 3 === 1 ? "bg-slate-500" : "bg-slate-700"}`} />
                      ))}
                    </motion.div>
                  </div>
                  <span className="mb-4 font-mono text-xs font-bold leading-relaxed text-center text-slate-400">
                    {cubeSolved ? "CUBE ALGORITHM SOLVED" : "BRUTE FORCE SCANNING..."}
                  </span>
                  <button onClick={() => setCubeSolved(!cubeSolved)} className="px-3 py-1.5 rounded-md bg-white/5 border border-white/10 text-[10px] font-semibold text-slate-500 hover:text-slate-200 transition-colors w-full active:scale-95">
            ...
            <LogbookContainer title={algorithmData.word_bn} subtitle={algorithmData.word_en} date={algorithmData.engineering_logbook.date} itemVariants={itemVariants}>
              <LogbookItem number="১" icon={HelpCircle} title={algorithmData.engineering_logbook.points[0].title}><p className="text-justify">{algorithmData.engineering_logbook.points[0].description}</p></LogbookItem>
              <LogbookItem number="২" icon={Hash} title={algorithmData.engineering_logbook.points[1].title}>
                <ul className="pt-1 space-y-2">
                  {algorithmData.engineering_logbook.points[1].description.split('•').filter(Boolean).map((listItem, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-justify md:text-base text-slate-300">
                      <span className="w-1.5 h-1.5 rounded-full bg-slate-500 mt-2 shrink-0" />
                      <span><strong className="text-slate-200">{listItem.split(':')[0]}:</strong>{listItem.split(':')[1]}</span>
                    </li>
                  ))}
                </ul>
              </LogbookItem>
              <LogbookItem number="৩" icon={Cpu} title={algorithmData.engineering_logbook.points[2].title}><p className="text-justify">{algorithmData.engineering_logbook.points[2].description}</p></LogbookItem>
              <LogbookItem number="৪" icon={Heart} title={algorithmData.engineering_logbook.points[3].title} isHighlight={true}><p className="font-serif leading-relaxed text-slate-300">"{algorithmData.engineering_logbook.points[3].description}"</p></LogbookItem>
            </LogbookContainer>
            ...
            <motion.div variants={itemVariants} className="p-4 md:p-6 rounded-xl border border-white/10 bg-[#0b111b] shadow-md space-y-4 font-sans">
              <h3 className="flex items-start gap-2 text-base font-bold text-slate-100 md:text-lg">
                ...
                <span className="leading-snug">{algorithmData.readers_reflection.title}</span>
              </h3>
              <p className="font-serif text-sm leading-relaxed text-justify whitespace-pre-line md:text-base text-slate-300">{algorithmData.readers_reflection.question}</p>
            ...
            <motion.div variants={itemVariants} className="p-4 md:p-6 rounded-xl border border-white/10 bg-[#0b111b] font-sans">
              <div className="space-y-3">
                <span className="text-xs sm:text-sm md:text-base font-mono font-bold tracking-widest text-slate-400 uppercase flex items-center gap-1 mb-2">
                  <Award size={12} className="text-slate-400" /> NEXT STEP INGREDIENT
                </span>
                <p className="text-xs leading-relaxed md:text-sm text-slate-300">{algorithmData.next_intro.text}</p>
                <div className="pt-2">
                  <button onClick={() => { setActiveTab('lab'); document.querySelector("[data-reader-scroll]")?.scrollTo?.({ top: 0, behavior: 'smooth' }); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="w-full sm:w-auto inline-flex justify-center items-center gap-2 px-5 py-2.5 rounded-lg bg-[#1f3a46] text-slate-100 font-bold text-sm hover:bg-[#294957] transition-all group active:scale-95">
                    ল্যাব সিমুলেটরে নুডলস রাঁধুন <ChevronRight size={14} className="transition-transform group-hover:translate-x-1"/>
                  </button>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants}>
              <WordNavigation fallbackPath="algorithm" />
            </motion.div>
