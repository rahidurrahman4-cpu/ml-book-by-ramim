        ...
        {activeTab === 'reading' ? (
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-12 text-[#c6c5d4]">
            ...
            <motion.div variants={itemVariants} className="pb-4 space-y-2 border-b border-white/5">
              <div className="text-xs font-bold text-[#00daf3] tracking-wide uppercase">{algorithmData.chapter} / {algorithmData.part}</div>
              <h1 className="flex items-center gap-2 text-3xl font-extrabold text-white">
                <GitCommit className="text-[#00daf3]" size={28} /> {algorithmData.word_bn} <span className="text-lg font-normal text-[#8080a0]">({algorithmData.word_en})</span>
              </h1>
            </motion.div>

            <motion.div variants={itemVariants} className="grid items-start grid-cols-1 gap-8 md:grid-cols-12">
              <div className="space-y-4 md:col-span-7">
                <div className="flex items-center gap-2 text-[#d846ef] font-bold text-xs uppercase tracking-wider"><Sparkles size={16} />{algorithmData.real_world_flash.title}</div>
                {algorithmData.real_world_flash.paragraphs.map((p, idx) => <p key={idx} className="text-lg text-justify indent-6">{p}</p>)}
              </div>
              <div className="flex justify-center pt-2 md:col-span-5">
                <div className="w-full max-w-[280px] rounded-2xl bg-[#070512] border border-white/5 p-5 shadow-xl flex flex-col items-center">
                  <div className="w-28 h-28 border border-dashed border-[#00daf3]/30 flex flex-col items-center justify-center p-2 rounded-xl mb-4">
                    <motion.div
                  ...
                  <span className="text-xs font-mono font-bold text-[#00daf3] text-center mb-4">
            ...
            <motion.div variants={itemVariants} className="p-6 rounded-xl border border-[#d846ef]/20 bg-gradient-to-r from-[#4f46e5]/3 to-[#d846ef]/3 shadow-md space-y-4">
              <h3 className="flex gap-2 text-lg font-bold text-white"><Sparkles className="text-[#d846ef]" size={18}/>{algorithmData.readers_reflection.title}</h3>
              <p className="font-serif italic whitespace-pre-line">{algorithmData.readers_reflection.question}</p>
            ...
            <motion.div variants={itemVariants} className="p-6 rounded-xl border border-[#00daf3]/20 bg-[#1c0c35]">
              ...
              <p className="text-[#c6c5d4] mb-3 italic">{algorithmData.next_intro.text}</p>
