    ...
    <div className="w-full space-y-8 font-sans text-slate-200">
      ...
      <div className="pb-4 space-y-3 text-center">
        <h2 className="flex items-center justify-center gap-3 text-2xl font-bold text-white md:text-3xl">
          <span className="px-3 py-1 text-xs text-teal-400 border rounded-full bg-teal-500/20 border-teal-500/30">ল্যাব-০৯</span>
      ...
      <div className="grid items-stretch grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-5 bg-white/[0.02] p-6 rounded-2xl border border-white/5 flex flex-col justify-between shadow-lg">
          <div>
              ...
              {ingredients.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-2.5 rounded-xl text-[11px] font-bold border transition-all flex items-center justify-between ${
                      isAdded
                  ...
                  >
                    <span className="flex items-center gap-2.5 text-left">
                      ...
                      <span>
                        ...
                        <span className="block text-[9px] text-teal-400/80 font-normal mt-0.5">{item.mlExplain}</span>
                      </span>
                    </span>
                    <span className="text-[10px] text-slate-500 font-normal">যোগ করুন +</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="bg-[#0b0f19] p-5 rounded-2xl border border-white/5 text-center relative h-52 flex flex-col justify-center items-center shadow-inner">
            {blenderStatus === 'idle' && (
              <div className="space-y-3">
                ...
                <p className="text-[11px] text-slate-400">ব্লেন্ডার খালি। ইনপুট ডেটা যোগ করুন।</p>
                <div className="flex flex-wrap justify-center gap-2">
                  {blenderIngredients.map((item) => (
                    <button
                      key={item.id}
                      onClick={() => handleRemoveIngredient(item.id)}
                      className="bg-teal-500/10 border border-teal-500/20 px-2 py-1 rounded-lg text-[10px] text-teal-300 font-bold hover:bg-rose-500/20 hover:border-rose-500/30 hover:text-rose-400 flex items-center gap-1 transition-colors"
                    >
                      {item.icon} {item.name} ×
                    </button>
                  ))}
            ...
            {blenderStatus === 'blending' && (
              <div className="space-y-3">
                ...
                <p className="text-[11px] text-teal-400 font-bold tracking-widest uppercase">প্রসেসিং চলছে...</p>
              </div>
            )}
            ...
            {blenderStatus === 'ready_good' && (
              <div className="space-y-2 animate-fade-in">
                ...
                <p className="text-sm font-bold text-emerald-400">সফল আউটপুট: সুস্বাদু জুস!</p>
                <p className="text-[10px] text-slate-400">সঠিক ও পরিষ্কার ইনপুট ডেটা ব্যবহার করা হয়েছে।</p>
              </div>
            )}
            ...
            {blenderStatus === 'ready_garbage' && (
              <div className="space-y-2 animate-fade-in">
                <div className="text-6xl">🤢🤮</div>
                <p className="text-sm font-bold text-rose-400">GIGO এফেক্ট: পচা নোংরা জুস!</p>
                <p className="text-[10px] text-rose-300/80">পচা ফল ইনপুট দেওয়ায় আউটপুটও নষ্ট হয়েছে।</p>
              </div>
            )}
            ...
            {blenderStatus === 'broken' && (
              <div className="space-y-2 animate-fade-in">
                <div className="text-6xl">💥🪨</div>
                <p className="text-sm font-bold text-red-500">সিস্টেম ক্র্যাশ! ব্লেন্ডার ভেঙে গেছে।</p>
                <p className="text-[10px] text-slate-400">কারণ ইনপুটে পাথর বা নয়েজ দেওয়া হয়েছে।</p>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2 mt-6">
            <button
              ...
              className="flex-[2] py-3 bg-gradient-to-r from-teal-600 to-teal-500 hover:from-teal-500 text-white text-xs font-bold rounded-xl shadow-lg transition-transform hover:scale-[1.02] disabled:opacity-50"
            >
        ...
        <div className="lg:col-span-7 bg-white/[0.02] p-6 rounded-2xl border border-white/5 flex flex-col justify-between gap-6 shadow-lg">
          <div>
            <h3 className="flex items-center gap-2 pb-2 mb-2 text-base font-bold text-white border-b border-white/5">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="bg-[#0b0f19] p-4 rounded-xl border border-white/5 text-center min-h-52 flex flex-col justify-center items-center overflow-hidden shadow-inner">
                <span className="text-[10px] uppercase tracking-widest font-bold text-slate-500 mb-4 border-b border-white/5 pb-1">মানুষ যা দেখে</span>
                ...
                {dataTypeView === 'image' && (
                  <div className="flex flex-col items-center w-full space-y-3">
                    ...
                    <button onClick={() => setDrawGrid(Array(25).fill(0))} className="text-[9px] text-teal-400 hover:text-teal-200">
                      গ্রিড পরিষ্কার করুন
                    </button>
                  </div>
                )}
                ...
                {dataTypeView === 'text' && (
                  <div className="w-full px-4 space-y-3 text-center">
                    <input
                      ...
                      className="w-full bg-white/5 border border-white/10 p-2.5 rounded-lg text-xs text-center text-teal-300 font-bold focus:outline-none focus:border-teal-500 transition-colors"
                    />
                    <p className="text-[9px] text-slate-500">লেখা পরিবর্তন করে ডানে লাইভ টোকেন দেখুন।</p>
                  </div>
                )}
                ...
                {dataTypeView === 'numeric' && (
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] text-slate-400">মানুষের দেহের তাপমাত্রা</p>
                  </div>
                )}
                ...
                {dataTypeView === 'audio' && (
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-slate-400 font-medium">"Siri, weather update please!"</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-[#0b0f19] p-4 rounded-xl border border-white/5 text-center min-h-52 flex flex-col justify-center items-center font-mono overflow-hidden shadow-inner relative">
                ...
                <span className="text-[10px] uppercase tracking-widest font-bold text-teal-500 mb-4 border-b border-teal-500/20 pb-1 z-10">মেশিনে যা পৌঁছায়</span>

                {dataTypeView === 'image' && (
                  <div className="text-[9px] text-teal-300 bg-teal-500/5 p-2 rounded-lg border border-teal-500/20 w-full leading-relaxed z-10">
                    [<br />
                    ...
                    ]<span className="text-[8px] text-teal-500/60 font-sans block mt-1 uppercase tracking-widest">Pixel Matrix</span>
                  </div>
                )}

                {dataTypeView === 'text' && (
                  <div className="z-10 w-full p-4 text-xs text-teal-300 border rounded-lg bg-teal-500/5 border-teal-500/20">
                    <p className="mb-2 text-[9px] text-teal-500 font-sans uppercase tracking-widest">ASCII Character ID:</p>
                    <p className="font-bold leading-relaxed tracking-widest text-white break-words">[ {textToTokens(inputText).join(', ')} ]</p>
                  </div>
                )}

                {dataTypeView === 'numeric' && (
                  <div className="z-10 p-4 text-sm text-teal-300 border rounded-lg bg-teal-500/5 border-teal-500/20">
                    <p className="text-2xl font-bold tracking-widest text-white">38.50000</p>
                    <span className="text-[9px] text-teal-500 font-sans block mt-2 uppercase tracking-widest">Float Value</span>
                  </div>
                )}

                {dataTypeView === 'audio' && (
                  <div className="text-[10px] text-teal-300 bg-teal-500/5 p-3 rounded-lg border border-teal-500/20 w-full z-10">
                    ...
                    <p className="text-white text-[9px] font-bold">[44.1 kHz, Amp: 0.88]</p>
                    <span className="text-[8px] text-teal-500/70 font-sans block mt-1 uppercase tracking-widest">Sound Frequency Data</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-teal-500/10 p-4 rounded-xl text-[11px] text-teal-200 leading-relaxed border border-teal-500/20">
