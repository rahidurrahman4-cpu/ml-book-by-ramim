    ...
    <div className="max-w-[1440px] mx-auto px-4 sm:px-5 md:px-16 py-5 md:py-8 space-y-6 md:space-y-8 text-[#dfe0ff]">
      ...
      <div className="flex flex-col gap-3 pb-2 border-b border-white/10 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-2 sm:gap-4">
          ...
          <button onClick={() => setActiveTab('lab')} className={`relative flex items-center gap-2 pb-2 font-bold text-[13px] sm:text-sm transition-colors ${activeTab === 'lab' ? 'text-teal-400' : 'text-[#8080a0]'}`}>
        ...
        {activeTab === 'reading' ? (
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-10 md:space-y-12 text-[#c6c5d4]">
            ...
            <motion.div variants={itemVariants} className="pb-4 space-y-2 border-b border-white/5">
              <div className="text-[11px] sm:text-xs font-bold text-[#00daf3] tracking-wide uppercase">{inputDataContent.chapter} / {inputDataContent.part}</div>
              <h1 className="flex flex-wrap items-center gap-2 text-2xl sm:text-3xl font-extrabold text-white">
                <FileDigit className="text-[#00daf3]" size={24} /> {inputDataContent.word_bn} <span className="text-sm sm:text-lg font-normal text-[#8080a0]">({inputDataContent.word_en})</span>
              </h1>
            </motion.div>

            <motion.div variants={itemVariants} className="grid items-start grid-cols-1 gap-6 md:gap-8 md:grid-cols-12">
              <div className="space-y-4 md:col-span-7">
                <div className="flex items-center gap-2 text-[#d846ef] font-bold text-[11px] sm:text-xs uppercase tracking-wider"><Sparkles size={15} />{inputDataContent.real_world_flash.title}</div>
                {inputDataContent.real_world_flash.paragraphs.map((p, idx) => <p key={idx} className="text-[14px] sm:text-lg text-justify leading-relaxed indent-5 sm:indent-6">{p}</p>)}
              </div>
              <div className="flex justify-center pt-2 md:col-span-5">
                <div className="w-full max-w-[280px] rounded-2xl bg-[#070512] border border-white/5 p-4 sm:p-5 shadow-xl flex flex-col items-center">
                  ...
                  <span className={`text-[11px] sm:text-xs font-mono font-bold text-center whitespace-pre-line leading-relaxed mb-4 h-8 ${blenderRunning ? 'text-[#d846ef]' : 'text-[#00daf3]'}`}>
            ...
            <motion.div variants={itemVariants} className="p-4 sm:p-6 rounded-xl border border-[#d846ef]/20 bg-gradient-to-r from-[#4f46e5]/3 to-[#d846ef]/3 shadow-md space-y-4">
              <h3 className="flex gap-2 text-base sm:text-lg font-bold text-white"><Sparkles className="text-[#d846ef] mt-0.5" size={17}/>{inputDataContent.readers_reflection.title}</h3>
              <p className="font-serif italic leading-relaxed whitespace-pre-line text-[14px] sm:text-base">{inputDataContent.readers_reflection.question}</p>
            ...
            <motion.div variants={itemVariants} className="p-4 sm:p-6 rounded-xl border border-[#00daf3]/20 bg-[#1c0c35]">
              ...
              <p className="text-[13px] sm:text-base text-[#c6c5d4] mb-3 italic leading-relaxed">{inputDataContent.next_intro.text}</p>
