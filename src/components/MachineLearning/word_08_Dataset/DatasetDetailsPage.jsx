          ...
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-10 md:space-y-12 font-sans text-base sm:text-lg md:text-xl lg:text-[21px] leading-relaxed text-slate-300">
            ...
            <motion.div variants={itemVariants} className="pb-3 space-y-1 font-sans border-b md:pb-4 md:space-y-2 border-white/5">
              <div className="flex flex-wrap items-center gap-1.5 md:gap-2 text-[10px] md:text-xs font-bold text-slate-500 tracking-wide uppercase">
                <span className="hidden sm:inline">{datasetData.part}</span>
              </div>
              <h1 className="flex flex-wrap items-center gap-2 text-2xl font-extrabold text-slate-100 sm:text-3xl">
                <Database className="w-6 h-6 text-slate-400 md:w-7 md:h-7 shrink-0" /> 
                {datasetData.word_bn} <span className="font-sans text-sm font-normal sm:text-lg text-slate-500">({datasetData.word_en})</span>
              </h1>
            </motion.div>

            {/* --- Real World Flash & GIGO Simulator --- */}
            <motion.div variants={itemVariants} className="grid items-start grid-cols-1 gap-6 md:gap-8 lg:grid-cols-12">
              <div className="space-y-3 lg:col-span-7 md:space-y-4">
                <div className="flex items-center gap-2 font-sans text-slate-400">
                  ...
                  <span className="text-sm font-bold leading-snug tracking-wider text-white uppercase sm:text-base md:text-lg">{datasetData.real_world_flash.title}</span>
                </div>
                {datasetData.real_world_flash.paragraphs.map((p, idx) => <p key={idx} className="text-justify indent-6">{p}</p>)}
              </div>
              <div className="flex justify-center pt-2 lg:col-span-5">
                <div className="w-full max-w-[280px] rounded-2xl bg-[#0b111b] border border-white/10 p-5 shadow-xl flex flex-col items-center mx-auto">
                  <div className="flex flex-col items-center justify-center p-2 mb-4 transition-all border border-dashed w-28 h-28 border-slate-500/30 rounded-xl">
                    {gigoMode ? <Trash2 size={36} className="text-red-400 animate-bounce sm:text-[40px]" /> : <Database size={36} className="text-green-400 sm:text-[40px]" />}
                    <span className="text-[10px] uppercase font-mono mt-2 text-slate-300 block">
                      {gigoMode ? "Garbage Data" : "Clean Dataset"}
                    </span>
                  </div>
                  <span className={`mb-4 font-mono text-xs font-bold leading-relaxed text-center whitespace-pre-wrap h-8 ${gigoMode ? 'text-red-400' : 'text-green-400'}`}>
                    {gigoMode ? "OUTPUT: GARBAGE\nModel Accuracy: 12%" : "OUTPUT: EXCELLENT\nModel Accuracy: 98%"}
                  </span>
                  <button onClick={() => setGigoMode(!gigoMode)} className="px-3 py-1.5 rounded-md bg-white/5 border border-white/10 text-[10px] font-semibold text-slate-500 hover:text-slate-200 transition-colors w-full active:scale-95">
            ...
            <LogbookContainer title={datasetData.word_bn} subtitle={datasetData.word_en} date={datasetData.engineering_logbook.date} itemVariants={itemVariants}>
              <LogbookItem number="১" icon={Database} title={datasetData.engineering_logbook.points[0].title}><p className="text-justify">{datasetData.engineering_logbook.points[0].description}</p></LogbookItem>
              <LogbookItem number="২" icon={Layers} title={datasetData.engineering_logbook.points[1].title}>
                <div className="grid grid-cols-1 gap-3 mt-2 md:grid-cols-3">
                  {datasetData.engineering_logbook.points[1].description.split('\n').map((listItem, i) => (
                    <div key={i} className="bg-[#030712] border border-white/10 rounded-lg p-4 shadow-sm">
                      <strong className="block mb-1.5 text-sm font-bold text-slate-200">
                        {listItem.split(':')[0].replace('• ', '')}
                      </strong> 
                      <span className="text-sm text-slate-300">{listItem.split(':')[1]}</span>
                    </div>
                  ))}
                </div>
              </LogbookItem>
              <LogbookItem number="৩" icon={Trash2} title={datasetData.engineering_logbook.points[2].title}><p className="leading-relaxed text-justify"><strong className="text-slate-200">Garbage In, Garbage Out:</strong> {datasetData.engineering_logbook.points[2].description.split('(GIGO):')[1]}</p></LogbookItem>
              <LogbookItem number="৪" icon={Heart} title={datasetData.engineering_logbook.points[3].title} isHighlight={true}><p className="font-serif leading-relaxed text-slate-300">"{datasetData.engineering_logbook.points[3].description}"</p></LogbookItem>
            </LogbookContainer>
            ...
            <motion.div variants={itemVariants} className="p-4 md:p-6 rounded-xl border border-white/10 bg-[#0b111b] shadow-md space-y-4 font-sans">
              <h3 className="flex items-start gap-2 text-base font-bold text-slate-100 md:text-lg">
                ...
                <span className="leading-snug">{datasetData.readers_reflection.title}</span>
              </h3>
              <p className="font-serif text-sm leading-relaxed text-justify whitespace-pre-line md:text-base text-slate-300">{datasetData.readers_reflection.question}</p>
                    ...
                    {datasetData.readers_reflection.options.map(option => pollSelected === option.id && (
                      <div key={option.id} className="space-y-1.5">
                        <span className={`flex items-center gap-1 font-bold ${option.isCorrect ? 'text-green-400' : 'text-red-400'}`}>
                          ...
                          {option.isCorrect ? 'একদম ফাটাফাটি! সঠিক উত্তর!' : 'উত্তরটি সঠিক হয়নি!'}
                        </span>
            ...
            <motion.div variants={itemVariants} className="p-4 md:p-6 rounded-xl border border-white/10 bg-[#0b111b] font-sans">
              <div className="space-y-3">
                ...
                <p className="text-xs leading-relaxed md:text-sm text-slate-300">{datasetData.next_intro.text}</p>
                <div className="pt-2">
                  <button onClick={() => { setActiveTab('lab'); document.querySelector("[data-reader-scroll]")?.scrollTo?.({ top: 0, behavior: 'smooth' }); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="w-full sm:w-auto inline-flex justify-center items-center gap-2 px-5 py-2.5 rounded-lg bg-[#1f3a46] text-slate-100 font-bold text-sm hover:bg-[#294957] transition-all group active:scale-95">
                    শার্লকের ক্রাইম সিন ল্যাবে প্রবেশ করুন <ChevronRight size={14} className="transition-transform group-hover:translate-x-1"/>
                  </button>
                </div>
              </div>
            </motion.div>
