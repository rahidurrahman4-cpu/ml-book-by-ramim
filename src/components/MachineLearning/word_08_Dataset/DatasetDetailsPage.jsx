...
import { Database, Sparkles, BookOpen, ChevronRight, CheckCircle, XCircle, FlaskConical, Trash2, Layers, FileText, Heart, Compass } from 'lucide-react';
import { Link, useParams } from 'react-router-dom';

import datasetData from './Dataset.json';
...
export default function DatasetDetailsPage() {
  ...
  const [gigoMode, setGigoMode] = useState(false);
        ...
        {activeTab === 'reading' ? (
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-10 md:space-y-12 text-[#c6c5d4]">
            ...
            <motion.div variants={itemVariants} className="pb-4 space-y-2 border-b border-white/5">
              <div className="text-[11px] sm:text-xs font-bold text-[#00daf3] tracking-wide uppercase">{datasetData.chapter} / {datasetData.part}</div>
              <h1 className="flex flex-wrap items-center gap-2 text-2xl sm:text-3xl font-extrabold text-white">
                <Database className="text-[#00daf3]" size={24} /> {datasetData.word_bn} <span className="text-sm sm:text-lg font-normal text-[#8080a0]">({datasetData.word_en})</span>
              </h1>
            </motion.div>

            <motion.div variants={itemVariants} className="grid items-start grid-cols-1 gap-6 md:gap-8 md:grid-cols-12">
              <div className="space-y-4 md:col-span-7">
                <div className="flex items-center gap-2 text-[#d846ef] font-bold text-[11px] sm:text-xs uppercase tracking-wider"><Sparkles size={15} />{datasetData.real_world_flash.title}</div>
                {datasetData.real_world_flash.paragraphs.map((p, idx) => <p key={idx} className="text-[14px] sm:text-lg text-justify leading-relaxed indent-5 sm:indent-6">{p}</p>)}
              </div>
              <div className="flex justify-center pt-2 md:col-span-5">
                <div className="w-full max-w-[280px] rounded-2xl bg-[#070512] border border-white/5 p-4 sm:p-5 shadow-xl flex flex-col items-center">
                  <div className="w-24 h-24 sm:w-28 sm:h-28 border border-dashed border-[#00daf3]/30 flex flex-col items-center justify-center p-2 rounded-xl mb-4 transition-all">
                    {gigoMode ? <Trash2 size={36} className="text-red-400 animate-bounce sm:text-[40px]" /> : <Database size={36} className="text-emerald-400 sm:text-[40px]" />}
                    <span className="text-[10px] uppercase font-mono mt-2 text-white block">
                      {gigoMode ? "Garbage Data" : "Clean Dataset"}
                    </span>
                  </div>
                  <span className={`text-[11px] sm:text-xs font-mono font-bold text-center whitespace-pre-wrap leading-relaxed mb-4 h-8 ${gigoMode ? 'text-red-400' : 'text-emerald-400'}`}>
                    {gigoMode ? "OUTPUT: GARBAGE\nModel Accuracy: 12%" : "OUTPUT: EXCELLENT\nModel Accuracy: 98%"}
                  </span>
                  <button onClick={() => setGigoMode(!gigoMode)} className="px-3 py-1.5 rounded-md bg-white/5 border border-white/10 text-[10px] font-bold text-slate-400 hover:text-white transition-colors w-full">
                    {gigoMode ? "Inject Clean Data" : "Inject Corrupted Data (GIGO)"}
                  </button>
                </div>
              </div>
            </motion.div>

            <StoryDialogue title={datasetData.story_prose.title} dialogues={datasetData.story_prose.paragraphs.map(p => ({text: p}))} itemVariants={itemVariants} />
            <ComparisonTable tableData={datasetData.comparison_table} itemVariants={itemVariants} />

            <LogbookContainer title={datasetData.word_bn} subtitle={datasetData.word_en} date={datasetData.engineering_logbook.date} itemVariants={itemVariants}>
              <LogbookItem number="১" icon={Database} title={datasetData.engineering_logbook.points[0].title}><p>{datasetData.engineering_logbook.points[0].description}</p></LogbookItem>
              <LogbookItem number="২" icon={Layers} title={datasetData.engineering_logbook.points[1].title}>
                <div className="grid grid-cols-1 gap-3 mt-2 md:grid-cols-3">
                  {datasetData.engineering_logbook.points[1].description.split('\n').map((listItem, i) => (
                    <div key={i} className="bg-[#0d1117]/60 border border-white/10 rounded-lg p-4 shadow-sm">
                      <strong className={`text-sm block mb-1.5 ${i === 0 ? 'text-[#00daf3]' : i === 1 ? 'text-[#d846ef]' : 'text-amber-400'}`}>
              ...
              <LogbookItem number="৩" icon={Trash2} title={datasetData.engineering_logbook.points[2].title}><p className="leading-relaxed text-justify"><strong>Garbage In, Garbage Out:</strong> {datasetData.engineering_logbook.points[2].description.split('(GIGO):')[1]}</p></LogbookItem>
              <LogbookItem number="৪" icon={Heart} title={datasetData.engineering_logbook.points[3].title} isHighlight={true}><p className="italic font-serif text-[#d8b4fe]">"{datasetData.engineering_logbook.points[3].description}"</p></LogbookItem>
            </LogbookContainer>
            ...
            <motion.div variants={itemVariants} className="p-4 sm:p-6 rounded-xl border border-[#d846ef]/20 bg-gradient-to-r from-[#4f46e5]/3 to-[#d846ef]/3 shadow-md space-y-4">
              <h3 className="flex gap-2 text-base sm:text-lg font-bold text-white"><Sparkles className="text-[#d846ef] mt-0.5" size={17}/>{datasetData.readers_reflection.title}</h3>
              <p className="font-serif italic leading-relaxed whitespace-pre-line text-[14px] sm:text-base">{datasetData.readers_reflection.question}</p>
              <div className="grid grid-cols-1 gap-3 pt-2 sm:grid-cols-2 sm:gap-4">
                {datasetData.readers_reflection.options.map((option, idx) => (
              ...
              <AnimatePresence>
                {pollSelected && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="p-4 rounded-lg border bg-white/[0.01] border-white/10 text-[#c6c5d4] mt-4 text-[13px] sm:text-sm leading-relaxed">
                    {datasetData.readers_reflection.options.map(option => pollSelected === option.id && (
            ...
            <motion.div variants={itemVariants} className="p-4 sm:p-6 rounded-xl border border-[#00daf3]/20 bg-[#1c0c35]">
              <span className="text-[10px] font-mono font-bold tracking-widest text-[#00daf3] uppercase flex items-center gap-1 mb-2">
                <Compass size={12} className="animate-[spin_3s_linear_infinite]" /> NEXT STEP INGREDIENT
              </span>
              <p className="text-[13px] sm:text-base text-[#c6c5d4] mb-3 italic leading-relaxed">{datasetData.next_intro.text}</p>
              <button onClick={() => { setActiveTab('lab'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="inline-flex items-center gap-2 px-4 sm:px-5 py-2.5 bg-[#00daf3] text-[#001f24] rounded-lg text-[13px] sm:text-sm font-bold shadow-[0_0_12px_rgba(0,227,253,0.3)] hover:bg-[#9cf0ff] transition-all">
                শার্লকের ক্রাইম সিন ল্যাবে প্রবেশ করুন <ChevronRight size={14}/>
              </button>
            </motion.div>
