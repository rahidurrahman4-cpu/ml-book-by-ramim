...
import { Trash2, Sparkles, BookOpen, FlaskConical, Target, Layers, Heart, Compass, Zap, CheckCircle, XCircle, ChevronRight, AlertTriangle, ShieldCheck, RefreshCw, ArrowRight } from 'lucide-react';
import { useParams } from 'react-router-dom';

import gigoData from './GIGO.json';
...
export default function GIGODetailsPage() {
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16 py-4 md:py-8 space-y-6 md:space-y-8 text-slate-300 bg-[#070b12]">
      ...
      <div className="flex flex-col gap-3 pb-3 border-b sm:flex-row sm:items-center sm:justify-between border-white/10 md:pb-2">
        ...
        <div className="hidden sm:block text-[10px] md:text-xs font-mono tracking-widest text-slate-500 uppercase">ML Word by Word • Word 20</div>
      </div>
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'reading' ? (
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-10 md:space-y-12 font-sans text-base sm:text-lg md:text-xl lg:text-[21px] leading-relaxed">
            ...
            <StoryDialogue title={gigoData.story_prose.title} dialogues={gigoData.story_prose.paragraphs.map(p => ({ text: p }))} itemVariants={itemVariants} />
            <ComparisonTable tableData={gigoData.comparison_table} itemVariants={itemVariants} />

            <LogbookContainer title={gigoData.word_bn} subtitle={gigoData.word_en} date={gigoData.engineering_logbook.date} itemVariants={itemVariants}>
               {gigoData.engineering_logbook.points.map((pt, i) => (
                 <LogbookItem key={i} number={pt.id} title={pt.title} icon={i === 3 ? Heart : i === 1 ? Zap : Layers} isHighlight={i === 3}>
                    <p>{pt.description}</p>
                 </LogbookItem>
               ))}
            </LogbookContainer>
            ...
            <motion.div variants={itemVariants} className="pt-4">
              <WordNavigation fallbackPath="gigo-ml" />
            </motion.div>
