    ...
    <div className="max-w-[1440px] mx-auto px-5 md:px-16 py-6 md:py-8 space-y-8 text-[#dfe0ff]">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-white/10">
        <div className="flex gap-4">
          ...
          <button onClick={() => setActiveTab('lab')} className={`relative flex items-center gap-2 pb-2 font-bold text-sm transition-colors ${activeTab === 'lab' ? 'text-indigo-400' : 'text-[#8080a0]'}`}>
          ...
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-12 text-[#c6c5d4]">
            ...
            <motion.div variants={itemVariants} className="pb-4 space-y-2 border-b border-white/5">
              ...
              <h1 className="flex items-center gap-2 text-3xl font-extrabold text-white">
                <SlidersHorizontal className="text-[#00daf3]" size={28} /> {featureData.word_bn} <span className="text-lg font-normal text-[#8080a0]">({featureData.word_en})</span>
              </h1>
            </motion.div>

            <motion.div variants={itemVariants} className="grid items-start grid-cols-1 gap-8 md:grid-cols-12">
              <div className="space-y-4 md:col-span-7">
                <div className="flex items-center gap-2 text-[#d846ef] font-bold text-xs uppercase tracking-wider"><Sparkles size={16} /> {featureData.real_world_flash.title}</div>
                {featureData.real_world_flash.paragraphs.map((p, i) => <p key={i} className="text-lg text-justify indent-6">{p}</p>)}
              </div>
              <div className="flex justify-center pt-2 md:col-span-5">
                <div className="relative w-full max-w-[280px] rounded-2xl border border-white/10 bg-[#070512] flex flex-col items-center justify-center p-5 shadow-xl">
                  <div className="relative w-28 h-28 border border-dashed border-[#00daf3]/30 flex flex-col items-center justify-center rounded-xl bg-white/[0.02] mb-4">
                    {filterNoise ? <Filter size={40} className="text-[#00daf3] animate-pulse" /> : <Smartphone size={40} className="text-amber-400" />}
                    <span className="text-[10px] uppercase font-mono mt-3 text-white text-center px-2">{filterNoise ? "EXTRACTING FEATURES" : "RAW INPUT DATA"}</span>
                  </div>
                  <button onClick={() => setFilterNoise(!filterNoise)} className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-[10px] font-bold text-slate-300 hover:text-white hover:bg-white/10 transition-all w-full">
            ...
            <motion.div variants={itemVariants} className="p-6 rounded-xl border border-[#d846ef]/20 bg-gradient-to-r from-[#4f46e5]/3 to-[#d846ef]/3 shadow-md space-y-4">
              <h3 className="flex items-center gap-2 text-lg font-bold text-white"><Zap className="text-[#d846ef]" size={18}/> {featureData.readers_reflection.title}</h3>
              <p className="font-serif italic leading-relaxed whitespace-pre-line">{featureData.readers_reflection.question}</p>
            ...
            <motion.div variants={itemVariants} className="p-6 rounded-xl border border-[#00daf3]/20 bg-[#1c0c35]">
              ...
              <p className="text-[#c6c5d4] mb-3 italic">{featureData.next_intro.text}</p>
