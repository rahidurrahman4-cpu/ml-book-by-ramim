...
import { Fuel, Sparkles, BookOpen, FlaskConical, Target, Layers, Heart, Compass, Zap, CheckCircle, XCircle, ChevronRight, EyeOff, ShieldAlert, Table } from 'lucide-react';
import { useParams, Link } from 'react-router-dom';

import dataContent from './DataDependency.json';
...
export default function DataDependencyDetailsPage() {
  ...
  const [fuelStream, setFuelStream] = useState('standard');
      ...
      <div className="flex flex-col gap-3 pb-3 border-b sm:flex-row sm:items-center sm:justify-between border-white/10 md:pb-2">
        <div className="flex w-full gap-2 sm:gap-4 sm:w-auto">
          ...
          <button onClick={() => setActiveTab('lab')} className={`flex-1 sm:flex-none relative flex items-center justify-center sm:justify-start gap-2 pb-2 font-bold text-xs sm:text-sm transition-colors ${activeTab === 'lab' ? 'text-slate-100' : 'text-slate-500 hover:text-slate-300'}`}>
            ...
            {activeTab === 'lab' && <motion.div layoutId="activeTabUnderline" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#00daf3]" />}
          </button>
        </div>
        <div className="hidden sm:block text-[10px] md:text-xs font-mono tracking-widest text-slate-500 uppercase">ML Word by Word • Word 21</div>
      </div>
        ...
        {activeTab === 'reading' ? (
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-10 md:space-y-12 font-sans text-base sm:text-lg md:text-xl lg:text-[21px] leading-relaxed">
            ...
            <motion.div variants={itemVariants} className="pb-3 space-y-1 border-b md:pb-4 border-white/5">
              <div className="text-[10px] md:text-xs font-bold text-slate-500 uppercase">{dataContent.part}</div>
              <h1 className="flex flex-wrap items-center gap-2 text-2xl font-extrabold text-slate-100 sm:text-3xl lg:text-4xl">
                <Fuel className="w-6 h-6 text-[#00daf3] md:w-8 md:h-8" /> {dataContent.word_bn} <span className="text-sm font-normal text-slate-500 sm:text-lg">({dataContent.word_en})</span>
              </h1>
            </motion.div>

            <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div className="space-y-4 lg:col-span-7">
                <div className="flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-wider"><Sparkles size={16} className="text-[#00daf3]"/> {dataContent.real_world_flash.title}</div>
                {dataContent.real_world_flash.paragraphs.map((p, i) => <p key={i} className="text-justify indent-6">{p}</p>)}
              </div>
              <div className="lg:col-span-5 bg-[#0b111b] border border-white/5 rounded-3xl p-8 text-center relative overflow-hidden shadow-2xl">
                 ...
                 <div className="relative w-28 h-28 border border-dashed border-[#00daf3]/30 flex flex-col items-center justify-center mx-auto rounded-2xl bg-white/[0.01] mb-6">
                    {fuelStream === 'standard' ? <Fuel size={48} className="text-[#00daf3] animate-pulse" /> : <EyeOff size={48} className="text-red-400" />}
                    <span className="text-[10px] font-black uppercase tracking-widest mt-2">{fuelStream === 'standard' ? "Trained Inflow" : "Unknown Dialect"}</span>
                 </div>
                 <p className="text-xs font-mono text-indigo-400 h-8 mb-4">{fuelStream === 'standard' ? "Input: Standard Bangla\nResult: 100% Processing Clean" : "Input: Sylheti Regional Audio\nResult: 0% Match | Error"}</p>
                 <button onClick={() => setFuelStream(f => f === 'standard' ? 'sylheti' : 'standard')} className="w-full py-3 rounded-xl bg-white/5 border border-white/10 text-xs font-bold text-slate-400 hover:text-white transition-all active:scale-95">
                   {fuelStream === 'standard' ? 'Feed Sylheti Audio' : 'Feed Standard Bangla'}
                 </button>
              </div>
            </motion.div>

            <StoryDialogue title={dataContent.story_prose.title} dialogues={dataContent.story_prose.paragraphs.map(p => ({text: p}))} itemVariants={itemVariants} />
            <ComparisonTable tableData={dataContent.comparison_table} itemVariants={itemVariants} />

            <LogbookContainer title={dataContent.word_bn} subtitle={dataContent.word_en} date={dataContent.engineering_logbook.date} itemVariants={itemVariants}>
               {dataContent.engineering_logbook.points.map((pt, i) => (
                 <LogbookItem key={i} number={pt.id} title={pt.title} icon={i === 3 ? Heart : i === 1 ? ShieldAlert : i === 0 ? Table : Target} isHighlight={i === 3}>
                    <p>{pt.description}</p>
                 </LogbookItem>
               ))}
            </LogbookContainer>
            ...
            {/* Reflection Poll */}
            <motion.div variants={itemVariants} className="p-5 md:p-8 rounded-[2.5rem] border border-white/5 bg-[#0b111b] shadow-2xl space-y-6 relative overflow-hidden font-sans">
               <div className="absolute top-0 right-0 w-32 h-32 bg-[#00daf3]/5 blur-3xl rounded-full" />
               <h3 className="flex items-start gap-3 text-base font-bold text-slate-100 md:text-xl relative z-10"><Sparkles className="mt-1 text-amber-400 shrink-0" size={20}/> {dataContent.readers_reflection.title}</h3>
               <p className="font-serif text-sm italic leading-relaxed text-justify md:text-lg text-slate-300 relative z-10 whitespace-pre-line">{dataContent.readers_reflection.question}</p>
               <div className="grid grid-cols-1 gap-4 pt-2 md:gap-5 sm:grid-cols-2 relative z-10">
                  {dataContent.readers_reflection.options.map(opt => (
               ...
               <AnimatePresence>
                  {pollSelected && (
                    <motion.div initial={{opacity:0, height:0}} animate={{opacity:1, height:'auto'}} className="p-4 rounded-2xl border bg-white/[0.02] border-white/10 text-slate-300 mt-4 text-sm md:text-base">
                      {dataContent.readers_reflection.options.map(o => o.id === pollSelected && <p key={o.id} className="leading-relaxed"><span className={`font-black uppercase mr-2 ${o.isCorrect ? 'text-emerald-400' : 'text-rose-400'}`}>{o.isCorrect ? 'Correct!' : 'Incorrect!'}</span>{o.explanation}</p>)}
                    </motion.div>
                  )}
               </AnimatePresence>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="p-6 md:p-10 rounded-[2.5rem] border border-white/5 bg-gradient-to-br from-[#0b111b] to-[#070b12] relative overflow-hidden group">
               ...
               <div className="relative z-10 space-y-4 text-slate-400">
                  <span className="text-[10px] md:text-xs font-mono font-bold tracking-[0.3em] uppercase flex items-center gap-2 mb-2"><Compass size={14} className="animate-spin-slow text-indigo-400" /> Entering The Prejudice Zone</span>
                  <p className="text-base italic leading-relaxed md:text-lg">{dataContent.next_intro.text}</p>
                  <div className="pt-4">
                    <button onClick={() => { setActiveTab('lab'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="w-full sm:w-auto inline-flex justify-center items-center gap-3 px-8 py-4 rounded-2xl bg-white text-[#070b12] font-black text-sm hover:bg-slate-200 transition-all active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.1)]">ল্যাব সিমুলেটরে গাড়ি চালান <ChevronRight size={18} className="group-hover:translate-x-1 transition-transform"/></button>
                  </div>
               </div>
            </motion.div>

            <motion.div variants={itemVariants} className="pt-4">
              <WordNavigation fallbackPath="data-dependency" />
            </motion.div>
