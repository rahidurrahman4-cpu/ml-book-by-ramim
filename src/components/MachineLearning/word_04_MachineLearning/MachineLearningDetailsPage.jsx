        ...
        {activeTab === 'reading' ? (
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-12 text-[#c6c5d4]">
            ...
            <motion.div variants={itemVariants} className="pb-4 space-y-2 border-b border-white/5">
              <div className="text-xs font-bold text-[#00daf3] tracking-wide uppercase">{mlData.chapter} / {mlData.part}</div>
              <h1 className="flex items-center gap-2 text-3xl font-extrabold text-white">
                <Brain className="text-[#00daf3]" size={28} /> {mlData.word_bn} <span className="text-lg font-normal text-[#8080a0]">({mlData.word_en})</span>
              </h1>
            </motion.div>

            <motion.div variants={itemVariants}>
              <div className="flex items-center gap-2 text-[#d846ef] font-bold text-xs uppercase tracking-wider mb-4"><Sparkles size={16} />{mlData.real_world_flash.title}</div>
              {mlData.real_world_flash.paragraphs.map((p, idx) => <p key={idx} className="mb-4 text-lg text-justify indent-6">{p}</p>)}
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="p-6 rounded-xl border border-[#d846ef]/20 bg-gradient-to-r from-[#4f46e5]/3 to-[#d846ef]/3 shadow-md space-y-4">
              <h3 className="flex gap-2 text-lg font-bold text-white"><Sparkles className="text-[#d846ef]" size={18}/>{mlData.readers_reflection.title}</h3>
              <p className="font-serif italic whitespace-pre-line">{mlData.readers_reflection.question}</p>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {mlData.readers_reflection.options.map((option, idx) => (
                  <button key={idx} onClick={() => setPollSelected(option.id)} className={`p-4 rounded-xl border text-left flex gap-2.5 ${pollSelected === option.id ? (option.isCorrect ? 'bg-green-500/10 border-green-500/40 text-white animate-pulse' : 'bg-red-500/10 border-red-500/40 text-white') : 'bg-white/5 border-white/10 text-slate-400'}`}>
                    ...
                    <div><span className="block font-bold text-white">অপশন {idx + 1}</span>{option.text}</div>
                  </button>
                ))}
              </div>
              <AnimatePresence>
            ...
            <motion.div variants={itemVariants} className="p-6 rounded-xl border border-[#d846ef]/20 bg-[#1c0c35]">
              <p className="text-[#c6c5d4]">উপরে থাকা <strong>"🔬 ল্যাব সিমুলেটর"</strong> ট্যাবে ক্লিক করে ম্যাজিক ইকুয়েশন এবং বিড়াল-কুকুর লার্নিং টেস্ট করে দেখুন!</p>
