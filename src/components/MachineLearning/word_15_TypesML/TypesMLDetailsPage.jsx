...
import { Brain, Sparkles, BookOpen, FlaskConical, Target, Layers, Heart, Compass, Zap, CheckCircle, XCircle, ChevronRight } from 'lucide-react';
import { useParams } from 'react-router-dom';

import typesData from './types_ml.json';
        ...
        {activeTab === 'reading' ? (
          <motion.div key="reading" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} className="space-y-10 md:space-y-12 font-sans text-base sm:text-lg md:text-xl lg:text-[21px] leading-relaxed text-slate-300">
            ...
            <motion.div variants={itemVariants} className="pb-3 space-y-1 font-sans border-b md:pb-4 md:space-y-2 border-white/5">
              ...
              <h1 className="flex flex-col items-start gap-2 text-2xl font-extrabold sm:flex-row sm:flex-wrap sm:items-center sm:gap-2 sm:text-3xl text-slate-100">
                ...
                <span className="flex flex-wrap items-center gap-2 leading-tight">
                  {typesData.word_bn} <span className="font-sans text-sm font-normal sm:text-lg text-slate-500">({typesData.word_en})</span>
                </span>
              </h1>
            </motion.div>
            ...
            <LogbookContainer title={typesData.word_bn} subtitle={typesData.word_en} date={typesData.engineering_logbook.date} itemVariants={itemVariants}>
              {typesData.engineering_logbook.points.map((pt, i) => (
                <LogbookItem key={i} number={pt.id || (i + 1).toLocaleString('bn-BD')} title={pt.title} icon={i === 3 ? Heart : i === 2 ? Zap : i === 0 ? Target : Layers} isHighlight={i === 3}>
            ...
            {/* --- Next Chapter Teaser (Teaching the Machine) --- */}
            <motion.div variants={itemVariants} className="p-4 md:p-6 rounded-xl border border-white/10 bg-[#0b111b] font-sans">
              <div className="space-y-3">
                <span className="text-[9px] md:text-[10px] font-mono font-bold tracking-widest text-[#d846ef] uppercase flex items-center gap-1 mb-2">
                  <Compass size={12} className="text-[#d846ef] animate-pulse" /> TEACHING THE MACHINE
                </span>
                <p className="text-xs leading-relaxed md:text-sm text-slate-300">
                  {typesData.next_intro?.text}
                </p>
                <div className="pt-2">
            ...
            <motion.div variants={itemVariants}>
              <WordNavigation fallbackPath="types-ml" />
            </motion.div>
