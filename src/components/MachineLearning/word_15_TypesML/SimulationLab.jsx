  ...
  return (
    <div className="w-full space-y-8 font-sans text-slate-200">
      <div className="pb-4 space-y-3 text-center">
        ...
        <p className="max-w-2xl mx-auto text-sm text-slate-400">মেশিন লার্নিংয়ের ৩টি ভিন্ন ভিন্ন পদ্ধতি হাতে-কলমে পরীক্ষা করুন।</p>
      </div>
      ...
      <div className="flex flex-wrap justify-center bg-white/[0.02] p-1.5 rounded-xl border border-white/5 max-w-2xl mx-auto w-full gap-2 shadow-lg">
        <button onClick={() => setActiveTab('supervised')} className={`flex-1 py-2 px-3 rounded-lg text-xs font-bold transition-all ${activeTab === 'supervised' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}>👨‍🏫 সুপারভাইজড</button>
        <button onClick={() => setActiveTab('unsupervised')} className={`flex-1 py-2 px-3 rounded-lg text-xs font-bold transition-all ${activeTab === 'unsupervised' ? 'bg-emerald-600 text-white' : 'text-slate-400'}`}>🔍 আনসুপারভাইজড</button>
        <button onClick={() => setActiveTab('reinforcement')} className={`flex-1 py-2 px-3 rounded-lg text-xs font-bold transition-all ${activeTab === 'reinforcement' ? 'bg-fuchsia-600 text-white' : 'text-slate-400'}`}>🤖 রিইনফোর্সমেন্ট</button>
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'supervised' && (
            <motion.div key="sup" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="bg-[#1e2430] p-8 rounded-3xl border border-indigo-500/30 shadow-2xl space-y-6">
              <div className="flex items-center justify-between pb-4 border-b border-white/5">
                 <h3 className="text-xl font-bold text-indigo-400">স্প্যাম ডিটেক্টর (Supervised)</h3>
                 <span className="px-3 py-1 font-mono text-sm text-indigo-300 border rounded-full bg-indigo-950 border-indigo-500/30">Score: {supScore}</span>
              </div>
              <p className="text-sm italic text-slate-400">"নিচের মেসেজটি পড়ে সঠিক লেবেল নির্বাচন করুন। এখানে আপনিই সিস্টেমের শিক্ষক।"</p>
              
              <div className="bg-[#12161f] p-6 rounded-2xl border border-gray-700 text-center">
                 <p className="mb-6 text-lg font-bold text-white">"অভিনন্দন! আপনি ১০০০০ টাকা লটারি জিতেছেন। এখনই ক্লেইম করুন!"</p>
              ...
              {supFeedback && (
                <div className={`p-4 rounded-xl text-center font-bold ${supFeedback.status==='correct'?'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30':'bg-rose-500/20 text-rose-400 border border-rose-500/30'}`}>
                  {supFeedback.msg}
                </div>
              )}
            </motion.div>
          ...
          {activeTab === 'unsupervised' && (
            <motion.div key="unsup" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="bg-[#1e2430] p-8 rounded-3xl border border-emerald-500/30 shadow-2xl space-y-6">
              <h3 className="pb-4 text-xl font-bold border-b text-emerald-400 border-white/5">ডেটা ক্লাস্টারিং (Unsupervised)</h3>
              <p className="text-sm text-slate-400">"নিচে একগাদা অগোছালো ডেটা পয়েন্ট আছে। 'গ্রুপ করুন' বাটন চাপলে মেশিন মিল দেখে আলাদা করবে।"</p>
              ...
              <button onClick={() => setClustered(!clustered)} className={`w-full py-4 rounded-xl font-black transition-all ${clustered ? 'bg-gray-800 text-white' : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg'}`}>
          ...
          {activeTab === 'reinforcement' && (
            <motion.div key="rl" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="bg-[#1e2430] p-8 rounded-3xl border border-fuchsia-500/30 shadow-2xl space-y-6">
              <div className="flex items-center justify-between pb-4 border-b border-white/5">
                 <h3 className="text-xl font-bold text-fuchsia-400">রোবট ট্রেনিং (Reinforcement)</h3>
                 <span className={`text-sm font-bold ${battery<30?'text-rose-500 animate-pulse':'text-emerald-400'}`}>🔋 চার্জ: {battery}%</span>
              </div>
              ...
              <div className="p-4 font-mono text-xs italic text-center border bg-black/20 rounded-xl border-white/5 text-slate-300">
                "{rlLog}"
              </div>

              <div className="flex gap-4">
                 <button onClick={handleRLMove} disabled={battery <= 0 || robotPos >= 100} className="flex-1 py-4 font-bold text-white transition-all shadow-lg bg-fuchsia-600 hover:bg-fuchsia-500 disabled:opacity-30 rounded-xl active:scale-95">
